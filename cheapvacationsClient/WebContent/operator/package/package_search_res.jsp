...
<body>
	...
	<div class="wrapper">
		...
		<br /> <br />
		<% if(op.getPackages() == null ){ %>
			<br /><strong>NO Results Found</strong>		
		<% } else if(op.getPackages().isEmpty()) { %>
			<br /><strong>NO Results Found</strong>
		<% } else { %>		
		...
		<blockquote>
			...
			<div id="hotel-details" style="float: left">
				<%  String ho_id = request.getParameter("ho_id");
					...
					if( ho_id != null ) {
						if( ho_id.length() > 0 ) {
							HotelOffer ho = op.getHotelOffer();
						...
						if(fo_id.length() > 0) {
							FlightOffer fo = op.getFlightOffer();
			...
			<%
				int k = 1;
				Iterator<VacationPackage> i = op.getPackages().iterator();
				while(i.hasNext()){
					...
					out.print("<br />");
					out.print("<p>Travel Agency: "+tempp.getReferenceAgency().getUsername()+"</p>");
					out.print("<br />");
